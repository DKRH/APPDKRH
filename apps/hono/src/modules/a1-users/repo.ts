import { a_user, a_account } from "@dkrh/db/schema";

import { db, eq, and, ilike, or, desc } from "@dkrh/db";

export async function getAll(search: string, offset: number, limit: number) {
  const conditions = search
    ? or(ilike(a_user.name, `%${search}%`), ilike(a_user.email, `%${search}%`))
    : undefined;
...
export async function findCredentialAccount(userId: string) {
  const [account] = await db
    ...
    .from(a_account)
    .where(
      and(eq(a_account.userId, userId), eq(a_account.providerId, "credential")),
    )
    .limit(1);
...
export async function updatePassword(accountId: string, passwordHash: string) {
  const [account] = await db
    ...
    .where(eq(a_account.id, accountId))
    .returning({
      id: a_account.id,
      userId: a_account.userId,
    });
