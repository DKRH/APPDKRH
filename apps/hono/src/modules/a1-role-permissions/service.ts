...
import { a1_role_permission, a1_permission, a1_system } from "@dkrh/db/schema";
...
export async function getRolePermissions(roleId: string) {
  return db
    .select({
      ...
      permissionId: a1_permission.id,

      permissionCode: a1_permission.code,

      permissionName: a1_permission.name,

      systemId: a1_system.id,
    ...
    .innerJoin(
      a1_permission,
      eq(a1_role_permission.permissionId, a1_permission.id),
    )
    .innerJoin(a1_system, eq(a1_permission.systemId, a1_system.id))
    .where(eq(a1_role_permission.roleId, roleId));
}
...
export async function removePermission(roleId: string, permissionId: string) {
  const [result] = await db
    ...
    .where(
      and(
        eq(a1_role_permission.roleId, roleId),
        eq(a1_role_permission.permissionId, permissionId),
      ),
    )
    .returning();
