...
function pickItem(items: GachaItem[], rarity: number) {
...
function rollItem(
  ...
  const fourStarEarlySafety = nextPity4 === hard4 - 1 && nextPity5 === hard5;

  const bannerItemIds = new Set(itemInBanner.map((item) => item.id));

  const itemOffBanner = itemAll.filter((item) => !bannerItemIds.has(item.id));
  let is_featured = "";
  // ----------------------------------------
  // 5★
  // ----------------------------------------
  ...
  if (roll5 < chance5 || fiveStarHardPity) {
    ...
    if (percentageWinGuarantee5 || guarantee5 === 1) {
  ...
  // 4★
  // ----------------------------------------
  else if (roll4 < chance4 || fourStarHardPity || fourStarEarlySafety) {
    rarity = 4;

    nextPity4 = 0;

    const percentageWin4 = Math.random() < 0.75;
...
export async function pull(c: Context) {
  ...
  const itemInBanner = await repo.getBannerItems(bannerId);
  const itemAll = await repo.getItems();

  if (itemInBanner.length === 0) {
    return c.json(
      {
        message: "Banner has no items",
      },
      400,
    );
  }

  const currentPity = await repo.getOrCreateUserPity();

  let pity5 = currentPity.pity5 ?? 0;

  let pity4 = currentPity.pity4 ?? 0;

  let guarantee5 = currentPity.guarantee5 ?? 0;
...
export async function getBanners(c: Context) {
  const data = await repo.getBanners();
...
export async function getBanner(c: Context) {
  ...
  const banner = await repo.getBanner(id);
  ...
  const items = await repo.getBannerItems(id);
...
export async function getPity(c: Context) {
  const bannerId = c.req.param("id");
...
export async function getHistory(c: Context) {
  ...
  const data = await repo.getUserHistory(limit, offset);
