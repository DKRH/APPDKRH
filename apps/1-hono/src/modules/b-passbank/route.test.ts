...
describe("passbank routes", () => {
  ...
  describe("GET /", () => {
    ...
    test("supports query parameters", async () => {
      ...
      await app.request("/?search=github&offset=10&limit=20");
  ...
  describe("GET /:id", () => {
    test("calls getById", async () => {
      ...
      const response = await app.request("/passbank-1");
  ...
  describe("PUT /:id/restore", () => {
    test("calls restoreData", async () => {
      ...
      const response = await app.request("/passbank-1/restore", {
        method: "PUT",
      });
  ...
  describe("DELETE /:id/forever", () => {
    test("calls deleteDataForever", async () => {
      ...
      const response = await app.request("/passbank-1/forever", {
        method: "DELETE",
      });

      expect(response.status).toBe(200);
      expect(deleteDataForever).toHaveBeenCalledTimes(1);
    });
  });
});
