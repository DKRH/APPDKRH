...
import { getAuditContext } from "@/db/audit/context";

// ============================================================
// NOTES
// ============================================================

export async function findAllNotes(options?: {
  ...
  const conditions = [eq(dNotes.createdBy, context.userId)];

  if (view === "trash") {
    conditions.push(eq(dNotes.isTrashed, true));
  } else if (view === "archive") {
    conditions.push(eq(dNotes.isArchived, true), eq(dNotes.isTrashed, false));
  } else {
    conditions.push(eq(dNotes.isArchived, false), eq(dNotes.isTrashed, false));
  }

  if (search) {
    conditions.push(
      or(
        ilike(dNotes.title, `%${search}%`),
        ilike(dNotes.content, `%${search}%`),
      )!,
    );
  }
  ...
  return db
    ...
    .orderBy(desc(dNotes.isPinned), desc(dNotes.updatedAt));
}
...
export async function findNoteById(noteId: string) {
  ...
  return db
    .select()
    .from(dNotes)
    .where(and(eq(dNotes.id, noteId), eq(dNotes.createdBy, context.userId)))
    .limit(1);
}
...
export async function setPinned(noteId: string, isPinned: boolean) {
  const context = getAuditContext();
  const [note] = await db
    .update(dNotes)
    .set({
...
export async function findAllLabels() {
  ...
  return db
    .select()
    .from(dLabels)
    .where(eq(dLabels.createdBy, context.userId))
...
export async function findLabelById(labelId: string) {
  ...
  return db
    .select()
    .from(dLabels)
    .where(and(eq(dLabels.id, labelId), eq(dLabels.createdBy, context.userId)))
    .limit(1);
}
...
export async function findLabelsByNote(noteId: string) {
  const context = getAuditContext();
  return db
    .select({
      id: dLabels.id,
    ...
    .from(dJNoteLabels)
    .innerJoin(dLabels, eq(dJNoteLabels.labelId, dLabels.id))
    .where(
      and(
...
export async function findNotesByLabel(labelId: string) {
  const context = getAuditContext();
  return db
    .select({
      id: dNotes.id,
    ...
    .from(dJNoteLabels)
    .innerJoin(dNotes, eq(dJNoteLabels.noteId, dNotes.id))
    .where(
      and(
...
export async function addLabelToNote(noteId: string, labelId: string) {
  ...
  const [relation] = await db
    ...
    .onConflictDoNothing({
      target: [dJNoteLabels.noteId, dJNoteLabels.labelId],
    })
    .returning();
