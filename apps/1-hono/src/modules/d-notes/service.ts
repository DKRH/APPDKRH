...
export async function getAll(c: Context) {
  try {
    const view = (c.req.query("view") ?? "notes") as
      "notes" | "archive" | "trash";

    const search = c.req.query("search");

    const data = await repo.findAllNotes({
      view,
      search,
    });
...
export async function getOne(c: Context) {
  try {
    ...
    const labels = await repo.findLabelsByNote(noteId);
...
export async function createData(c: Context) {
  try {
    ...
    const color = typeof body.color === "string" ? body.color : "bg-zinc-900";
...
export async function editData(c: Context) {
  try {
    ...
    const note = await repo.updateNote(noteId, update);
...
export async function togglePin(c: Context) {
  try {
    ...
    const updated = await repo.setPinned(noteId, !note.isPinned);
...
export async function toggleArchive(c: Context) {
  try {
    ...
    const updated = await repo.setArchived(noteId, !note.isArchived);
...
export async function restore(c: Context) {
  try {
    ...
    const note = await repo.restoreNote(noteId);
...
export async function deleteData(c: Context) {
  try {
    ...
    const note = await repo.moveNoteToTrash(noteId);
...
export async function deleteForever(c: Context) {
  try {
    ...
    const note = await repo.deleteNoteForever(noteId);
...
export async function getLabels(c: Context) {
  try {
    const data = await repo.findAllLabels();
...
export async function createLabel(c: Context) {
  try {
    ...
    const label = await repo.createLabel(name);
...
export async function updateLabel(c: Context) {
  try {
    ...
    const label = await repo.updateLabel(labelId, name);
...
export async function deleteLabel(c: Context) {
  try {
    ...
    const label = await repo.deleteLabel(labelId);
...
export async function addLabel(c: Context) {
  try {
    ...
    const relation = await repo.addLabelToNote(noteId, labelId);

    return c.json(
      {
        message: relation ? "Label added" : "Label already attached",
        data: relation,
      },
      relation ? 201 : 200,
    );
...
export async function removeLabel(c: Context) {
  try {
    ...
    const relation = await repo.removeLabelFromNote(noteId, labelId);

    if (!relation) {
      return c.json(
        {
          message: "Label is not attached to this note",
        },
        404,
      );
    }
