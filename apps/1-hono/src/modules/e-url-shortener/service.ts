...
export async function createData(c: Context) {
  ...
  const data = await repo.create({
    ...
    expireDateUTC: body.expireDateUTC ? new Date(body.expireDateUTC) : null,
...
export async function changeLock(c: Context) {
  ...
  const updated = await repo.updateLock(id, newLockedState);

  return c.json({
    message: newLockedState ? "Short URL locked" : "Short URL unlocked",
...
export async function restoreData(c: Context) {
  ...
  const data = await repo.restore(id);
