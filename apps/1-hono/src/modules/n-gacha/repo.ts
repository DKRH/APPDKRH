...
import { and, eq, isNull, desc } from "@/db";
...
export async function getBanners() {
  return db
    .select({
      ...
      hardPityStart: banner.hardPityStart,
      uprate5: banner.uprate5,
      uprate4: banner.uprate4,
    })
    .from(banner)
    .where(isNull(banner.deletedAt))
...
export async function getBannerItems(bannerId: string) {
  return db
    .select({
      id: item.id,
      name: item.name,
    ...
    .from(bannerItem)
    .innerJoin(item, eq(bannerItem.itemId, item.id))
    .where(
      and(
        eq(bannerItem.bannerId, bannerId),
        isNull(bannerItem.deletedAt),
...
export async function getUserPity() {
  ...
  const [result] = await db
    .select()
    .from(pity)
    .where(and(eq(pity.userId, context.userId), isNull(pity.deletedAt)))
    .limit(1);
...
export async function getOrCreateUserPity() {
  const existing = await getUserPity();
...
export async function updatePity(data: {
  pity5: number;
  pity4: number;
  guarantee5: number;
}) {
...
export async function getUserHistory(limit = 50, offset = 0) {
  ...
  return db
    .select()
    .from(history)
    .where(and(eq(history.userId, context.userId), isNull(history.deletedAt)))
    .orderBy(desc(history.obtainedAt))
    .limit(limit)
    .offset(offset);
}
